...
fs.readFile('transfer.json', 'utf8', function(err, transfer) {
    ...
    fs.readFile(`${jsonFile}`, `${readingWay}`, function(err, json) {
        ...
        fs.readFile(`./CODE/${mC.FileName}.spss`, `${readingWay}`, function(err, data) {
            ...
            fs.writeFile(`./CODE/${mC.GeneratedFileName}.css`, data2, function(err) {
                if (err) {
                    console.log(err);
                } else {
                    if (mC.GeneratedLogs === true) {
                        newLine('Successfuly generated the CSS.\nYou can now use it for your code.');
                    }
                }
            });
        });
